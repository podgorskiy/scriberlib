...
typedef FT_FILE* (*ft_fopen_t)(const char* filename, const char* mode);
typedef int(*ft_fclose_t)(FT_FILE* stream);
typedef size_t(*ft_fread_t)(void* ptr, size_t size, size_t count, FT_FILE* stream);
typedef int(*ft_fseek_t)(FT_FILE* stream, long int offset, int origin);
typedef long int(*ft_ftell_t)(FT_FILE* stream);

#ifdef __cplusplus
extern "C" {
#endif

	void ft_set_file_callback(ft_fopen_t fnOpen, ft_fclose_t fnClose, ft_fread_t fnRead, ft_fseek_t fnSeek, ft_ftell_t fnTell);
